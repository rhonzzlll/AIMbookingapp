...
const Users = () => {
  ...
  const [passwordSuccess, setPasswordSuccess] = useState("");
   const departments = [
    ...
    { label: "Departments", options: ["SRF", "MRG", "OD", "ICT", "HRS", "FSG", "OR","ACC"] }
  ];
  ...
  const [reportFilter, setReportFilter] = useState('all'); // 'all', 'active', 'inactive'

  const handleDownloadCSV = () => {
    const headers = ['First Name', 'Last Name', 'Email', 'Department', 'Status'];
    let filtered = users;
    if (reportFilter === 'active') filtered = users.filter(u => u.isActive);
    if (reportFilter === 'inactive') filtered = users.filter(u => !u.isActive);
    ...
    link.href = url;
    link.setAttribute('download', `users_report_${reportFilter}.csv`);
          ...
          <div className="flex items-center gap-2">
            ...
            {[...Array(totalPages)].map((_, idx) => (
              <button
                ...
                className={`px-2 py-1 rounded ${currentPage === idx + 1 ? 'bg-blue-600 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}
              >
                {idx + 1}
              </button>
            ))}
